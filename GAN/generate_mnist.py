...
mnist = datasets.MNIST(
    root=DATA_PATH, train=True, transform=img_transform, download=True
)
...
for epoch in range(num_epoch):  # 进行多个epoch的训练
    for i, (img, _) in enumerate(dataloader):
        ...
        if epoch == 0:
            ...
            save_image(real_images, SAVE_PATH + '/real_images.png')
    fake_images = to_img(fake_img.cpu().data)
    save_image(fake_images, SAVE_PATH + '/fake_images-{}.png'.format(epoch + 1))

# 保存模型
torch.save(G.state_dict(), SAVE_PATH + '/generator.pth')
torch.save(D.state_dict(), SAVE_PATH + '/discriminator.pth')
